...
static void prvSetupTimerInterrupt( void )
{
	...
	//TickTimer_Enable();
    CRGINT_RTIE=1; /*Ϊ1ʱ�����ж����� */
    RTICTL = 0x8F;//0x1F; /* set RTI prescaler */ /*ÿ1ms����һ��ʵʱ�ж�*/
    CRGINT = 0x80; /* enable RTI interrupts  �ж�ʹ�ܼĴ��� */		 
}
/*-----------------------------------------------------------*/
 ...
 * handler.
 */
void interrupt 118 vPortYield( void )
{
	portSAVE_CONTEXT();
	vTaskSwitchContext();
	portRESTORE_CONTEXT();
 ...
 */
void interrupt 7 vPortTickInterrupt( void )
{
